---
slides:
    title: Estágio Supervisionado FJP - AID/SPLOR-MG.
---

# Estágio supervisionado FJP 2026

<!--s-->

## Apresentações

- Quem sou eu?
- Quem é você!!
- Quem somos nós...

<!--s-->

## Objetivo da conversa

Avaliar menos o conhecimento prévio e mais a disposição em se adaptar e refletir sobre o papel da tecnologia na administração pública.

Notes:
- Assessoria atua fortemente com dados, tecnologia e métodos de trabalho pouco tradicionais no setor público.
- Não queremos um programador.
- Queremos um futuro administrador público com letramento digital e analítico.
- Não é necessário saber programar, mas precisa estar disposto a aprender, experimentar e trabalhar de uma forma diferente do padrão burocrático clássico.
- Não se assuste.

<!--s-->

## AID

[Decreto de competência](https://www.almg.gov.br/legislacao-mineira/texto/DEC/48636/2023/#:~:text=processos%20da%20Subsecretaria.-,Art.%2036,-%E2%80%93%20A%20Assessoria%20de)

Na prática nosso maior papel é transformar dados em informação útil para tomada de decisão.

Notes:
Isso envolve desde:
- [Link para relatório operacional](http://200.198.9.184/Qlikview/index.htm).
- Pensar como os dados são organizados.
- Construir ferramentas (ETL).
- Automatizar análises (ainda não chegamos lá).
- Ensinar "todos" a usar esses recursos, sendo assim,programação aqui não é um pré-requisito, é uma competência que será construída.

<!--s-->

## Forma de trabalho

Boa parte da nossa comunicação não acontece por reuniões ou mensagens instantâneas, mas por **registros escritos**.

Notes:
- [GitHub](https://github.com/).
- [Issues e Planner](https://github.com/orgs/splor-mg/projects/13/views/20).
- E-mails não, [notificações](https://github.com/notifications).
- Importância de escrever para explicar raciocínios e registrar decisões.

<!--v-->

## Filosofia docs as code

Documentação faz parte de todo trabalho e não deve ser algo deixado somente para o final.

Tudo que é aprendido, decidido ou construído deve ficar registrado.

Notes:
- Utilização de ferramentas de desenvolvimento de software para construção de documentação:
    - Versionamento de código.
    - Revisão compartilhada.
    - Publicação automática.
- Para que isso se transforme em hábito é necessário prática, não se aprende da noite para o dia.

<!--s-->

## Monografia

???

<!--v-->

## Monografia - Sugestões

- Banco de dados relacional x grafos (ontologia orçamentária).
- Estrutura conjuntos de dados abertos orçamentários.
- Nova ferramenta de BI para substituir o Qlik view.
- Formatos mais eficientes para publicação/divulgação da LOA.
- Ganhos na utilização de projetos open sources para a administração pública.
- Ganhos na utilização da filosofia Docs as code para a administração pública.
